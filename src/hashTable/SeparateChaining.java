...
public class SeparateChaining<K, V> implements HashTable<K, V> {
    ...
    private double loadFactor;// to check if the table is full
    private int numberOfCollisions, totalCollisions;// collision number before and after rehasing
    ...
    public void put(K key, V value) {
        ...
        // above is the put method to insert into the linked list.
    }

    private void checkForRehashing() {
        //calculate load factor
        loadFactor = (double) numberOfEntries / tableSize;
        if (loadFactor >= 3) rehash();// to prompt rehashing
    }
    ...
    private int[] searchForEntry(K key) {
        ...
        for (int i = 0; i < chainSize; i++) {
            if (hashTable.get(homePosition).get(i) != null) {
